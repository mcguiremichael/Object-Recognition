...
class Agent():
    def __init__(self, action_size, num_skills, mode='PPO'):
        ...
        elif (mode == 'PPO_LSTM_MHDPA'):
            self.policy_net = PPO_LSTM_MHDPA(action_size, 64, 1)
            self.target_net = PPO_LSTM_MHDPA(action_size, 64, 1)
        
        
        self.policy_net.to(device)
        self.target_net.to(device)
        
        self.skill_discrim = Discriminator(num_kills).to(device)
    ...
    def get_action(self, state, skill_indices, hidden_state=None):
        ...
        if "LSTM" in self.mode:
            ...
            probs, val = self.policy_net(state, skill_indices)
