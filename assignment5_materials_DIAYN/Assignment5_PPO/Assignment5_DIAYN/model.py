...
class Discriminator(nn.Module):
    def __init__(self, num_skills):
        ...
        self.head = nn.Linear(512, self.num_skills)
    ...
    def forward(self, x):
        ...
        probs = F.softmax(x - torch.max(x,1)[0].unsqueeze(1))
        return probs
...
class PPO_MHDPA(nn.Module):
    def __init__(self, action_size, depth, num_skills=8, device="cuda:0"):
        super(PPO_MHDPA, self).__init__()
        self.action_size = action_size
        self.num_skills = num_skills
        self.conv1 = nn.Conv2d(depth+2, 32, kernel_size=5, stride=2, padding=2)
        #self.bn1 = nn.BatchNorm2d(32)
        self.conv2 = nn.Conv2d(32, 64, kernel_size=4, stride=2, padding=2)
        #self.bn2 = nn.BatchNorm2d(64)
        ...
        for i in range(len(num_skills)):
            self.output_layers.append(
                nn.Sequential(
                    nn.Linear(2304, 512),
                    nn.ReLU(),
                    nn.Linear(512, action_size+1)
                )
            )
    ...
    def forward(self, x, skill_indices=None):
        ...
        output = torch.zeros((x.size(0), self.action_size+1)).to(device)
        
        for i in range(len(self.num_skills)):
            indices = (skill_indices == i).nonzero().flatten()
            output[indices] = self.output_layers[i](x[indices])
...
class PPO_LSTM_MHDPA(nn.Module):
    def __init__(self, action_size, hidden_size=64, depth=1):
        super(PPO_LSTM_MHDPA, self).__init__()
        self.action_size = action_size
        self.conv1 = nn.Conv2d(depth, 32, kernel_size=8, stride=4, padding=4)
        #self.bn1 = nn.BatchNorm2d(32)
        self.conv2 = nn.Conv2d(32, 64, kernel_size=4, stride=2, padding=2)
    ...
    def forward(self, x, hidden):
        ...
        output, hidden = self.lstm(x, hidden)
        #x = self.conv4(output)
        x = self.attentionBlock1(output)
        x = self.attentionBlock2(output)
        x = F.leaky_relu(self.fc(x.view(x.size(0), -1)))
        x = self.head(x)
    ...
    def forward_unroll(self, x, hiddens, unroll_steps=1):
        ...
        for i in range(unroll_steps):
            outputs, hiddens = self.lstm(features[:,i], hiddens)
        #x = self.conv4(outputs)
        x = self.attentionBlock1(outputs)
        x = self.attentionBlock2(outputs)
        x = F.leaky_relu(self.fc(x.view(x.size(0), -1)))
        x = self.head(x)
